...
jQuery(function($) {
	...
	$('form[data-form="registration"]').on('submit', function(e) {
		...
		var el = $(this);
      	//var formData = el.serializeObject();
      	var ktpData = $('input[name=ktp]').prop('files')[0];
      	var strukData = $('input[name=struk]').prop('files')[0];
      	var buktiData = $('input[name=bayar]').prop('files')[0];
      	var data = new FormData();
      	var formData = new FormData($(this)[0]);
		try {
			$.ajax({
				url: $('#site-url').val() + '/assets/php/registration.php',
		        type: 'POST',
		        data: formData,
		        async: false,
		        cache: false,
